...
Information regarding special features from some grid elements is given below.
...
Column grid elements provide a register `Adjustment` allowing to control collapsing from rows and columns.
...
elements used in grid elements. The settings add related css-classes to the HTML code. Without any additional css
...
.. tip::
   ...
   A tile unit is basically a column with specific settings. You might use as well columns grid elements
...
The card grid element can render most card layouts as outlined on the `bootstrap framework page
